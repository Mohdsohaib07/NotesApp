...
//deleting a note
   async function deleteNote(id){
    try {
        ...
        if(confirmBox==true){
        const res = await axios.delete(`http://localhost:8080/note/${id}`,{headers:{
            "Authorization":token
          }});
...
async function addNewNote(data){
        try {
            const response = await axios.post('http://localhost:8080/note',data,{headers:{
                "Authorization":token
              }});
...
async function editNote(id,data) {
    try {
        const res = await axios.patch(`http://localhost:8080/note/${id}`,data,{headers:{
            "Authorization":token
        }});
        console.log(res.data);
        toast.success(res.data.message);
